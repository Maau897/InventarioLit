...
class SupabaseUsersConfig:
    ...
    audit_table_name: str = "inventario_auditoria"
...
def autenticar_usuario(email: str, password: str, normalizar_rol_fn):
    rows = _table().select("*").eq("email", email.strip().lower()).limit(1).execute().data or []
    if not rows:
...
def registrar_evento_auditoria(*, email: str, accion: str, detalle: str = "") -> None:
    _audit_table().insert(
        {
            "email": email.strip().lower(),
            "accion": accion.strip(),
...
def listar_eventos_auditoria(limit: int = 80) -> list[dict]:
    return (
        _audit_table()
        .select("id_evento,email,accion,detalle,created_at")
...
def crear_admin_inicial(email: str, password: str) -> None:
    ...
    existing = _table().select("id_usuario").eq("email", correo).limit(1).execute().data or []
    if existing:
